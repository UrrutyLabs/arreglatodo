...
import { paymentEventRepository } from "../repositories/paymentEvent.repo";
import { bookingRepository } from "../repositories/booking.repo";
import type { PaymentProviderClient } from "../payments/provider";
...
export class PaymentService {
  ...
  async createPreauthForBooking(
    ...
    if (booking.proProfileId) {
      const { proRepository } = await import("../repositories/pro.repo");
      const pro = await proRepository.findById(booking.proProfileId);
      if (pro) {
    ...
    try {
      ...
      await paymentRepository.updateStatusAndAmounts(payment.id, {
  ...
  async handleProviderWebhook(event: {
    ...
    const payment = await paymentRepository.findByProviderReference(
      event.provider,
    ...
    await paymentEventRepository.createEvent({
      paymentId: payment.id,
    ...
    if (newStatus !== payment.status) {
      ...
      if (newStatus === PaymentStatus.AUTHORIZED) {
        ...
        if (booking.status === BookingStatus.PENDING_PAYMENT) {
          await bookingRepository.updateStatus(payment.bookingId, BookingStatus.PENDING);
        }
      } else if (
  ...
  async syncPaymentStatus(paymentId: string): Promise<void> {
    ...
    const registryModule = await import("../payments/registry");
    const providerClient = await registryModule.getPaymentProviderClient(payment.provider);
